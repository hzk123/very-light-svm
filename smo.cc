...
double linear_kernel(vector<double> a, vector<double> b)
{
    return dot(a,b);    // linear kernel
}
...
int take_step(int _i,int _j) //implement SMO to update alpha1 and alpha2
{
    ...
    double s = y1 * y2;
    double E1, E2;
    if (alpha[_i] > 0 && alpha[_i] < C)
        E1 = error_cache[_i];
    else
        E1 = f(x[_i]) - y1;
    if (alpha[_j] > 0 && alpha[_j] < C)
        E2 = error_cache[_j];
    else
        E2 = f(x[_j]) - y2;
    ...
    {
        L=max(0.,alpha2+alpha1-C),H=min(C,alpha1+alpha2);
    }
...
//        cout << "???" << endl;
    ...
    if (  ( a2 < 0 || a2 > C ) && (a1 >=0 && a1 <=C) )
        b_new = b1_new;
    else if ( ( a1 >= 0 && a1 <=C ) && ( a2 < 0 || a2 > C) )
        b_new = b2_new;
    else
    ...
    if ((r1 < -TOLERANCE && alpha1 < C) || (r1 > TOLERANCE && alpha1 > 0))
    {
        ...
        for (i2 = -1, tmax = 0, k = 0; k < n; k++)
            if (alpha[k] > 0 && alpha[k] < C )
                if ( (temp = fabs(e1 - error_cache[k])) > tmax)
                    tmax = temp, i2 = k;
        if (i2 >= 0)
        {
            if  (take_step(i1, i2))
